...
def main():
    ...
    netA = torch.load("model_weight/netA_n_classes_10_label10_unlabel90_rand30.pth", map_location=device)
    netC = torch.load("model_weight/netC_n_classes_10_label10_unlabel90_rand30.pth", map_location=device)

    netA = netA.to(device)
    netC = netC.to(device)
    print("Models loaded and moved to GPU successfully.")
