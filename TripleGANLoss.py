...
def z_obeys_nakagami(data_label, bs_data_label,m=1):
    ...
    '''
    z = torch.zeros((bs_data_label,2048))
    for i in range(bs_data_label):
        p = torch.mean(data_label[i,:])
        z[i,:] = torch.tensor(nakagami.rvs(m, loc = 0, scale = abs(p), size = 2048))
...
if __name__ == "__main__":
    ...
    #监督数据
    data_of_data_label = torch.randn((10,2,2048))
    label_of_data_label = torch.ones((10))
    label_of_data_label = label_of_data_label.long()

    #无监督数据
    data_of_data = torch.randn((10,2,2048))
    ...
    loss_fn_A = Loss_of_AutoEncoder()
    loss_fn_G = Loss_of_Generator(device_num=0)
    loss_fn_C = Loss_of_Classifier()
    loss_fn_D = Loss_of_Discriminator(device_num=0)
